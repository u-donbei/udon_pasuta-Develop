 ...
 * @author 渡邉雄宇
 */
public class Onigiri extends Ally {
	/**
	 * 画像
	 */
	private static final File IMAGE = new File(PathConsts.TEXTURE_DIR.getPath() + "onigiri.png");
	/**
	 * 技
	 */
	...
	public void attack(Message message, Enemy... target) {
		...
		//メッセージの表示
		message.reText("おにぎりくんの" + technique.getName() + "攻撃!", null, 0.1);
	...
	public void move(Runnable process) {
		...
		Udon main = party.getLeader();

		//もし、パーティに参加していないならreturn
		if (!party.members().contains(this)) return;

		if (getX() < main.getX() && getX() < main.getX() - 60) {
			stepX(3);
		} else if (getX() > main.getX() && getX() > main.getX() + 60) {
			stepX(-3);
		}

		if (getY() < main.getY() && getY() < main.getY() - 60) {
			stepY(3);
		} else if (getY() > main.getY() && getY() > main.getY() + 60) {
			stepY(-3);
		}
	}
